    ...
    ui(new Ui::MainWindow),m_serial(new QSerialPort(this))
{
    ...
    //软件界面总体设计
    setWindowTitle(tr("永磁同步电机转速监控系统 V1.0 By Leonard"));
    ...
    //当前时间相关设置
    ui->timeEdit_PresentTime->setReadOnly(true);
    ui->timeEdit_PresentTime->setDisplayFormat("HH:mm:ss");
    ui->timeEdit_PresentTime->setTime(QTime::currentTime());
    ...
    customPlot->yAxis->setRange(-100, 100);//设置y轴范围
    ...
    connect(&dataTimer, SIGNAL(timeout()), this, SLOT(realtimeDataSlot()));//定时器连接槽函数realtimeDataSlot
...
void MainWindow::readData()
{
    ...
    if (!buffer.isEmpty())
    {
        QString temp_1=buffer;
        QString data_1=temp_1.replace("\r\n","");
        ...
        double value = data_1.toDouble();

        qDebug()<<"电机转速值为："<<value;

        if (key-lastPointKey > 0.1)//添加数据周期为100ms
        {
            customPlot->graph(0)->addData(key, 0);
            customPlot->graph(0)->addData(key, value);
            customPlot->graph(0)->rescaleValueAxis(true);//重新缩放数值轴以适应当前数据
            lastPointKey = key;
            customPlot->replot();
        }
        customPlot->xAxis->setRange(key, 5, Qt::AlignRight);//使时间轴范围与数据一起滚动

        if (ui->radioButton_ReceiveChar->isChecked())
        {
            QString temp_2=buffer;
            ui->plainTextEdit_Receive->appendPlainText(temp_2);
            //解决应用applendPlainText()后多添加一行问题
            ui->plainTextEdit_Receive->moveCursor(QTextCursor::End);//移动至新添加一行的末尾
            ui->plainTextEdit_Receive->textCursor().deletePreviousChar();//对新添加一行进行删除
            QString data_2=temp_2.replace("\r\n","");
            qDebug()<<"接收数据为:"<<data_2;
        }
